...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if DoesEntityExist(vehicle) and IsThisModelAHeli(GetEntityModel(vehicle)) then
            heli = vehicle -- Remember the helicopter for blade speed adjustment

            SetHeliBladesSpeed(heli, speed)

            if IsControlJustPressed(0, 172) then -- The keybinding setting (Current: ARROW DOWN)
                speed = speed + 0.005 -- The amount of engine power removed.
            elseif IsControlJustPressed(0, 173) then -- The keybinding setting (Current: ARROW UP)
                speed = speed - 0.005 -- The amount of engine power given.
            end

            if speed > 1.0 then speed = 1.0 end -- The maximum engine power & blade speed that can be reached to.
            if speed < 0.0 then speed = 0.0 end -- The minimum engine power & blade speed that can be reached to.
            ShowText(tostring(speed), 1, {255,255,255,255}, 0.5, 0.5, 0.5)
        elseif heli then
            -- Reset blade speed if player exits the helicopter
            SetHeliBladesSpeed(heli, 0.0) -- Once the pilot exits the cockpit, the blade speed is set to 0% (can be changed).
            speed = 0.0
            heli = nil
        end
    end
